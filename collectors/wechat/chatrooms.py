...
class WeChatChatroomsMixin(WeChatCollectorDeps):
    ...
    def _is_wechat_recent_overlay(self) -> bool:
        ...
        for sel in checks:
            try:
                ...
                continue
        # Title match alone is usually sufficient on this screen.
        return hit >= 1
    ...
    def _scroll_down_safe(self) -> bool:
        try:
            w, h = self.device.window_size()
            x = int(w * 0.5)
            # Keep the swipe short and away from the bottom gesture zone to avoid opening Recent.
    ...
    def _go_chat_list_top(self, max_swipes: int = 3) -> None:
        # Stop early when screen no longer changes (likely already at top).
        try:
            same_count = 0
    ...
    def _resolve_chat_tap_point(self, candidate: ChatCandidate) -> tuple[int, int, str]:
        # Re-resolve tap point from current viewport OCR to reduce coordinate drift.
        try:
            shot = self.device.screenshot()
            ...
            if matches:
                ...
                # Bias the tap to the left side of the title box instead of its center.
    ...
    def _retry_open_chat(self, candidate: ChatCandidate) -> bool:
        # Retry strategy when first tap does not enter chat room.
        try:
