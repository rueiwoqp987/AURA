...
def _build_record_id(chat_id: str, ordinal: int, raw_block: str) -> str:
    return hashlib.sha256(f"{chat_id}|record|{ordinal}|{raw_block}".encode("utf-8", errors="replace")).hexdigest()
...
def parse_whatsapp_export_text(chat_id: str, lines: Iterable[str], *, source_path: str) -> ParsedExport:
    ...
    def flush_current() -> None:
        nonlocal current_block_lines, current_ordinal, current_ts, current_sender, current_content, current_raw, current_type
        if not current_block_lines:
            return
        raw_block = "\n".join(current_block_lines).rstrip("\n")
        message_id = _build_message_id(chat_id, raw_block)
        record_id = _build_record_id(chat_id, current_ordinal, raw_block)
        out.append(
            {
                "record_id": record_id,
                "message_id": message_id,
                "observation_id": f"{record_id}:primary",
                "type": current_type,
    ...
    flush_current()
    return ParsedExport(messages=out, attachments=[], missing_attachments=[], source_txt_name=None)
...
def parse_whatsapp_export_zip(chat_id: str, zip_path: Path) -> ParsedExport:
    ...
    with ZipFile(str(zpath), "r") as zf:
        ...
        if not txt_names:
            return ParsedExport(messages=[], attachments=[], missing_attachments=[], source_txt_name=None)
    ...
    for msg in messages:
        ...
        if not matched:
            if attached_name:
                inferred_type = _infer_attachment_type(attached_name)
                msg["type"] = inferred_type
                msg["attachment_file_name"] = attached_name
                msg["attachment_missing"] = True
                msg["raw_source"] = msg.get("raw_source") or "whatsapp_export_text_attachment_missing"
                missing_attachments.append(
                    {
                        ...
                        "failure_reason": "zip_member_missing",
                    }
                )
            elif msg.get("type") == "Media":
                msg["attachment_missing"] = True
                msg["raw_source"] = msg.get("raw_source") or "whatsapp_export_media_omitted"
                missing_attachments.append(
                    {
                        "file_name": "<Media omitted>",
                        "message_id": msg.get("message_id"),
                        "record_id": msg.get("record_id", msg.get("message_id")),
                        "observation_id": msg.get("observation_id") or f"{msg.get('record_id', msg.get('message_id'))}:primary",
                        "message_type": "Media",
                        "failure_reason": "media_omitted_no_zip_member",
                    }
                )
            continue
    ...
    return ParsedExport(messages=messages, attachments=attachments, missing_attachments=missing_attachments, source_txt_name=txt_name)
