...
def get_wifi_connected(serial=None) -> bool:
    # Prefer explicit state outputs first.
...
def snapshot_network_state(serial=None) -> dict:
    ...
    # `dumpsys wifi` can include stale connected/SSID info even when Wi-Fi is disabled.
    # Treat Wi-Fi disabled as authoritative: not connected, no SSID.
    if wifi_enabled == "1":
...
def evaluate_network_policy(snapshot: dict, policy: dict) -> list[str]:
    ...
    if mode == "offline_airplane":
        ...
        # Intentionally skip Wi-Fi connectivity checks for phase1.
        # In this mode, airplane/DND controls are treated as the authoritative condition.
    ...
    if mode == "online_wifi":
        ...
        if require_dnd and str(snapshot.get("dnd_mode", "")) == "0":
            failures.append("dnd_not_enabled")
        # Phase2 gate is based on effective Wi-Fi connection status.
