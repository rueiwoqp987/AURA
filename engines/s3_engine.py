...
class S3Engine(BaseMethodEngine):
    method_id = "S3"

    def run(self, adapter, device, run_root, target_dir, audit_log_path, profile):
        collector = adapter.create_s3_collector(device=device, target_dir=target_dir, audit_log_path=audit_log_path)
        if collector is None:
            logger.info("S3 skipped: adapter %s has no S3 collector", adapter.app_key)
