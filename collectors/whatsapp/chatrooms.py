...
class WhatsAppChatroomsMixin(WhatsAppCollectorDeps):
        ...
        for direction in directions:
            for idx in range(max_swipes):
                ...
                self.log_action(
                    ...
                    selector="Bluetooth",
                    artifacts=[f"swipe_try={idx + 1}", f"direction={direction}"],
                )
    ...
    def _export_current_chat(self, chat_name: str, chat_id: str, export_dir) -> dict:
        ...
        try:
            ...
            # Register received export files and ingest messages from the export zip.
            for received_path in received_files:
                try:
                    ...
                    self.log_action(
                        ...
                        selector=chat_name,
                        artifacts=[str(p), f"messages={len(msgs)}", f"attachments={len(attachment_entries)}", f"txt={parsed.source_txt_name or ''}"],
                    )
                    ...
                    # WhatsApp PoC uses a single logical account.
