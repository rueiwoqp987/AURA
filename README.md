...
```powershell
python main.py --target telegram --serial <ADB_SERIAL> --runs-dir <OUTPUT_ROOT> --keep-run-dir --log-level DEBUG
```
...
| `--serial` | configured default | ADB device serial. Override this for the active test device. |
| `--runs-dir` | configured default | Output root for run directories, ZIP bundles, and summary reports. |
...
- Android/Samsung Settings screens vary across devices; DND and Bluetooth flows may require new node-tree examples on new models.
