...
class S2Engine(BaseMethodEngine):
    method_id = "S2"

    def run(self, adapter, device, run_root, target_dir, audit_log_path, profile):
        collector = adapter.create_s2_collector(device=device, target_dir=target_dir, audit_log_path=audit_log_path)
        if collector is None:
            logger.info("S2 skipped: adapter %s has no S2 collector", adapter.app_key)
