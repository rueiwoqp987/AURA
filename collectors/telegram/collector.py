...
class TelegramCollector(
    ...
    def _clear_recent_apps_for_phase(self, reason: str) -> None:
        """Mirror setup's recent-app cleanup before changing online phase state."""
        pkg = self.profile.get("package_name") or self.packageName
        ...
        self._sleep(float(self.profile.get("phase_recent_cleanup_settle_sec", 0.5)))

        try:
            close_all = self.device(text="Close all")
            if close_all.exists:
                close_all.click()
                self.log_action("clear_recent", selector="Close all", artifacts=[f"reason={reason}"])
            else:
                self.device.press("home")
                self.log_action("press", selector="home", artifacts=[f"reason={reason}", "close_all_not_found"])
        except Exception as e:
            self.log_action("clear_recent", selector="Close all", result="fail", error=e, artifacts=[f"reason={reason}"])
            try:
                self.device.press("home")
                self.log_action("press", selector="home", artifacts=[f"reason={reason}", "fallback_after_clear_recent_fail"])
            except Exception:
                pass
    ...
    def _restore_global_state(self, initial_state: dict) -> None:
        ...
        try:
            # Use the same UI-toggling path as phase DND enforcement.
            ret = ensure_dnd_mode(
                self.device,
