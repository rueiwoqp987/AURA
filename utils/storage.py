...
class SQLiteStorage:
    ...
    def _init_schema(self):
        ...
        self._ensure_column(cur, "acquisition_attempts", "audit_operation_id", "TEXT")
    ...
    def _init_views(self, cur):
        cur.executescript(
            ...
            WHERE m.message_type IN ('Photo', 'Video', 'File', 'Media')
            GROUP BY
                cc.phase,
                cc.account,
                cr.chat_name,
    ...
    def upsert_attachment_attempt(self, payload: Mapping):
        ...
        cur.execute(
            ...
            INSERT OR REPLACE INTO acquisition_attempts (
                ...
                bounds, snapshot_dirs, download_detection_method, audit_operation_id, artifact_ids, artifact_paths, sha256_list,
            ...
            (
                ...
                payload.get("download_detection_method"),
                payload.get("audit_operation_id"),
